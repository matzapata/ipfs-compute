...
func DeployCommand(privateKey string, provider string, pinataApiKey string, pinataSecret string, rpc string) {
	...
	// 	log.Fatal(err)
	// }
	// deployentRepo := deployment_repository.IpfsDeploymentsRepository()
	// deploymentService := deployment.NewDeploymentService(deployentRepo)
	...
	// if !console_helpers.Confirm("Do you want to deploy? (y/n): ") {
	// 	return // exit
	// }

	// // create signature
	// signature, err := ecdsa_helpers.SignMessage([]byte(config.TERMS_AND_CONDITIONS), privateKey)
	// if err != nil {
	// 	log.Fatal(err)
	// }
